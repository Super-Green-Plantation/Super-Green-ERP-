...
export const generateEmpNo = async (tx?: any) => {
  const db = tx || prisma;
  const members = await db.member.findMany({
    select: { empNo: true },
  });

  const existingEmpNos = new Set(members.map((m: any) => m.empNo?.trim()).filter(Boolean));
  ...
  for (const m of members) {
    if (!m.empNo) continue;
    const match = m.empNo.trim().match(/^(.*?)(\d+)$/);
    if (match) {
      const pfx = match[1];
      const num = parseInt(match[2], 10);
      // Ignore extreme outlier numbers (e.g. >= 100000 like test data 987655) to prevent jumping sequence
      if (num < 100000 && num > maxNum) {
        maxNum = num;
        if (pfx !== undefined && pfx !== "") prefix = pfx;
  ...
  let candidate = `${prefix}${String(candidateNum).padStart(padLen, "0")}`;

  // Guard 1: In-memory check against existing list
  while (existingEmpNos.has(candidate)) {
    candidateNum++;
    candidate = `${prefix}${String(candidateNum).padStart(padLen, "0")}`;
  }

  // Guard 2: Direct database query guard to guarantee 100% uniqueness
  let isUnique = false;
  while (!isUnique) {
    const existing = await db.member.findUnique({
      where: { empNo: candidate },
      select: { id: true },
    });
...
export async function getAutoGeneratedEmpNo() {
  return await generateEmpNo();
}
    ...
    const updated = await prisma.$transaction(async (tx) => {
      ...
      return await tx.member.update({
        ...
        data: {
          ...
          status: data.status,
          empNo: data.empNo && data.empNo.trim() !== "" ? data.empNo.trim() : (existing.empNo || await generateEmpNo(tx)),
          epfNo: data.epfNo,
          email: data.email || null,
          phone: data.phone || null,
          phone2: data.phone2 || null,
