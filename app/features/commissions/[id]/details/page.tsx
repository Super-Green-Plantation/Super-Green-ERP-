...
import {
  ArrowLeft, Printer, TrendingUp, ShieldCheck, User, Briefcase,
  Calendar, Wallet, MapPin, UserCheck, Pen, Trash2, ChevronRight, Loader2
} from "lucide-react";
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen gap-4">
        ...
        <p className="text-gray-500 font-bold tracking-tighter">Syncing Financial Data...</p>
      </div>
    );
  }

  if (!data) return <div className="p-10 text-center text-red-500">Investment record not found.</div>;
    ...
    <div className="max-w-7xl mx-auto space-y-6 min-h-screen p-4 md:p-6">
      ...
      <div className="flex items-center justify-between border-b pb-5">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
              Investment Receipt <span className="text-gray-400 font-medium">#{activeInvestment?.id || 'N/A'}</span>
            </h1>
        ...
        <div className="flex gap-2">
          <button className="hidden md:flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-xl text-sm font-bold text-gray-600 hover:bg-gray-50 transition-all">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              ...
              <div className="flex gap-8 border-l border-white/10 pl-8">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-400">{plan.rate}%</p>
                </div>
                <div>
                  <p className="text-[10px] text-gray-500 uppercase font-bold mb-1">Duration</p>
                  <p className="text-2xl font-bold">
                    {plan.duration} <span className="text-sm font-normal text-gray-400">Mo</span>
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="mt-2 flex items-center gap-2">
                <div className={`h-2.5 w-2.5 rounded-full animate-pulse ${client.status === 'Active' ? 'bg-green-500' : 'bg-red-500'}`} />
                <span className="text-lg font-bold text-gray-800">{client.status}</span>
              </div>
            </div>
            <div>
              <p className={labelStyle}>Frequency</p>
              <p className="text-lg font-bold text-gray-900">{activeInvestment?.returnFrequency || 'N/A'}</p>
            </div>
            <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <section className={cardBase}>
          ...
          <div className="p-6 grid grid-cols-2 gap-6">
            ...
            <div className="col-span-2 pt-2">
              <DetailItem label="Registered Address" value={client.address} icon={<MapPin className="w-3 h-3" />} />
            </div>
          </div>
        </section>
        ...
        <section className={cardBase}>
          <div className="px-6 py-4 bg-gray-50/50 border-b border-gray-100 flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-gray-800">Assigned Advisor</h2>
          </div>
          <div className="p-6">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-purple-600 font-bold uppercase tracking-tight">ID: {member.empNo}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4 border-t border-gray-50 pt-6">
              <DetailItem label="Branch" value={member.branch.name} icon={<MapPin className="w-3 h-3" />} />
              <DetailItem label="Branch Location" value={member.branch.location} />
            </div>
          </div>
        </section>
      </div>
      ...
      <div className="bg-red-50 border border-red-100 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          <h4 className="text-sm font-bold text-red-800">Termination Controls</h4>
          <p className="text-xs text-red-600 font-medium">Deactivating this client will pause all commission processing and reports.</p>
        </div>
