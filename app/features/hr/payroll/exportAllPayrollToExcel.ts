...
function toRow(r: PayrollRow) {
  return {
    ...
    "Other Deduction":      r.deductionAmount,
    "Deduction Remark":     r.deductionRemark,
    // ── Net ───────────────────────────────────────
    "Net Pay":              r.netPay,
  };
}
...
const NUM_COLS = [
  ...
  "Team Active","Excess Comm.","Fixed Allowance","Fuel Allowance","Attendance Allowance",
  "Channel Operation","Personal Comm.","ORC","Gross Pay",
  "EPF (Emp 8%)","EPF (Employer 12%)","ETF (3%)","Loan Instalments",
  "Festival Advance","Merchandise Deduction","Advance Deducted","Other Deduction","Net Pay",
];
...
const HEADER_ORDER = [
  ...
  "Channel Operation","Personal Comm.","ORC","Gross Pay",
  "EPF (Emp 8%)","EPF (Employer 12%)","ETF (3%)","Loan Instalments",
  "Festival Advance","Merchandise Deduction","Advance Deducted","Other Deduction","Deduction Remark",
  "Net Pay",
];

function colLetter(idx: number): string {
  // Supports A-Z and AA-ZZ (up to 52 cols)
...
function buildSheet(rows: PayrollRow[]): XLSX.WorkSheet {
  ...
  ws["!cols"] = [
    ...
    13, 14, 13, 20, 16,         // Excess … Channel Op
    14, 10,                     // Personal Comm … ORC
    13,                         // Gross Pay
    13, 17, 10,                 // EPF Emp … ETF
    14, 14, 20, 16,             // Loan … Advance
    15, 22,                     // Other Deduction … Remark
    12,                         // Net Pay
  ].map((w) => ({ wch: w }));
