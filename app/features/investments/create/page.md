...
import {
  ...
  FileText, ShieldCheck, UploadCloud, Image as ImageIcon,
} from "lucide-react";
              ...
              <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col gap-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {([
                    ...
                    const isSelected = !!docFiles[id];
                    const previewUrl = docPreviews[id];
                    const isPDF = docFiles[id]?.type === "application/pdf";
                    return (
                      <div
                        key={id}
                      ...
                      >
                        <input
                          ...
                          onChange={e => handleDocFileChange(id, e.target.files?.[0] || null)}
                        />
                        ...
                        {isSelected && (
                          <button
                            type="button"
                            onClick={e => { e.stopPropagation(); handleDocFileChange(id, null); }}
                            title="Remove file"
