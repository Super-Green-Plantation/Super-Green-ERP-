    ...
    <div className="mx-auto flex min-h-screen w-full max-w-[1480px] flex-col gap-6 px-4 pb-10 pt-5 sm:px-7 sm:pt-8">
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <section className="saas-surface min-h-[350px] overflow-hidden rounded-2xl p-5 sm:p-6 lg:col-span-2">
          <div className="mb-4 flex items-start justify-between"><div><h2 className="mt-1 text-base">Branch registrations</h2><p className="mt-1 text-[11px] text-muted-foreground">New registrations across your network</p></div><button type="button" className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground"><ArrowUpRight size={16} /></button></div>
        ...
        <section className="saas-surface min-h-[350px] overflow-hidden rounded-2xl">
          <div className="flex items-start justify-between border-b border-border/70 p-5 sm:p-6"><div><h2 className="mt-1 text-base">Recent activity</h2></div><Link href="/features/investments" className="text-[10px] font-bold text-primary hover:underline">View all</Link></div>
      ...
      <section><div className="mb-3 flex items-end justify-between"><div><h2 className="mt-1 text-lg">Network performance</h2></div><span className="text-[10px] font-semibold text-muted-foreground">Updated just now</span></div><BranchKpiTable /></section>

      <section><div className="mb-3"><h2 className="mt-1 text-lg">Maturity overview</h2></div>{maturityLoading || !maturityData ? <div className="saas-surface flex min-h-40 items-center justify-center rounded-2xl text-xs text-muted-foreground">Loading pipeline...</div> : <MaturityPipeline investments={maturityData} />}</section>

      <section><div className="mb-3"><h2 className="mt-1 text-lg">Team & finance</h2></div><div className="grid grid-cols-1 gap-5 lg:grid-cols-3"><CommissionLeaderboard /><IncentiveForecast /><PayrollBreakdown /></div></section>

      <section><div className="mb-3 flex items-end justify-between"><div><h2 className="mt-1 text-lg">Jump back in</h2></div><Link href="/features/profile" className="hidden items-center gap-1 text-[11px] font-bold text-primary sm:flex">Manage workspace <ChevronRight size={14} /></Link></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-3">{modules.map((mod) => { const Icon = mod.icon; return <Link key={mod.title} href={mod.href} className="group saas-surface flex items-center justify-between rounded-2xl p-4 transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-[0_16px_38px_rgba(34,43,72,0.09)]"><div className="flex items-center gap-3"><div className={`flex h-10 w-10 items-center justify-center rounded-xl ${mod.color}`}><Icon size={18} /></div><div><h3 className="text-sm font-bold">{mod.title}</h3><p className="mt-0.5 text-[10px] font-medium text-muted-foreground">{mod.description}</p></div></div><ArrowUpRight size={16} className="text-muted-foreground transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5 group-hover:text-primary" /></Link>; })}</div></section>
    </div>
  );
};
